...
namespace Leprechaun.Filters.Exclusions
{
	...
	public class ChildrenOfPathBasedPresetTreeExclusion : IPresetTreeExclusion
	{
		...
		public bool Evaluate(string itemPath)
		{
			...
			foreach (var exception in _exceptions)
			{
				...
				if (itemPath.StartsWith(unescapedException, StringComparison.OrdinalIgnoreCase)) return true;
			}
			...
			if (!itemPath.StartsWith(unescapedWildcardFreePath, StringComparison.OrdinalIgnoreCase)) return true;

			// if the path EQUALS the exclusion path it's included. Because we're including the root, and excluding the children.
			if (itemPath.Equals(unescapedWildcardFreePath, StringComparison.OrdinalIgnoreCase)) return true;
			...
			if (_excludeChildrenOfPath.EndsWith("/*/"))
			{
				...
				if (itemParentPath.Equals(unescapedWildcardFreePath, StringComparison.OrdinalIgnoreCase)) return true;
			}
