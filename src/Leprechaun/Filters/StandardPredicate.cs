...
{
	public abstract class StandardPredicate<T> : IFilterPredicate<T>, ITreeRootFactory where T : ItemInfoBase
	{
		private readonly string _rootNamespace;
		private readonly IList<ItemTreeRoot> _includeEntries;

		public StandardPredicate(XmlNode configNode, IContainer configuration, string rootNamespace)
		{
		...
		public virtual bool Includes(T item)
		{
		...
		/// <param name="item">Item to get a root NS for. MAY BE NULL, in which case a general root NS should be returned.</param>
		public virtual string GetRootNamespace(T item)
		{
		...
		/// </summary>
		protected virtual bool Includes(ItemTreeRoot entry, T itemData)
		{
		...
		protected virtual bool ExcludeMatches(ItemTreeRoot entry, T itemData)
		{
		...
		protected virtual IList<ItemTreeRoot> ParsePreset(XmlNode configuration)
		{
		...
		{
			// no entries = throw!
			if (_includeEntries.Count == 0) throw new InvalidOperationException($"No include entries were present on the predicate for the {configurationName} Leprechaun configuration. You must explicitly specify the items/templates you want to include.");
		}

		protected virtual ItemTreeRoot CreateIncludeEntry(XmlNode configuration)
		{
			...
			var root = new ItemTreeRoot(nameValue, path);
		...
		protected virtual IPresetTreeExclusion CreateExcludeEntry(XmlElement excludeNode, ItemTreeRoot root)
		{
